...
void get_price_range(milliseconds m, milliseconds fifteen_mins){
    ifstream read_data("C:/Users/Ethan/DatabaseProject/DatabaseInterface/IBM.csv");
    string line;
    milliseconds time(0);
...
// ======================== SIMULATION =====================================================================
    ifstream read_data("C:/Users/Ethan/DatabaseProject/DatabaseInterface/IBM.csv"); // note: there are 1353590 lines
    string line;
...
/*
    ...
    //Order sample_buy1(102, 12,10000,Stock, "IBM", nine_forty_AM, Market, Day, Buy, 153.86);
    //Order sample_buy2(103,4, 28, Stock, "IBM", nine_thirty_fiveAM, Market, Day, Buy, 0.1);
    Order sample_sell1(109,4, 25, Stock, "IBM", nine_forty_five_AM, Market, Day, Cover, 0.00); // this should increase cash balance by 100

    Order sample_order = sample_sell1;
*/
...
//========================================================================================================================
    ...
    //char response;
    //cout<<"Do you want to commit your updates(Y or N)?";
    //cin>>response;
...
int main(){
    ...
    //    Order sample_buy(1,2, 100, Stock, "IBM", milliseconds(100), Limit, Day, Buy, 153.85);
    //    Order sample_buy2(100009,1, 200, Stock, "IBM", milliseconds(200), Limit, Day, Buy, 153.84);
    //
    //    Order sample_ask(2,3, 25, Stock, "IBM", milliseconds(500), Market, Day, Sell, 153.80);
        Order sample_ask2(25,30, 100, Stock, "IBM", milliseconds(1111111), Market, Day, Sell, 153.84);
    ...
    OMS my_broker(All_ECNs);


    //my_broker.sample_SAConnectionFunction(con, sample_ask2);





    int num_of_offers = 0, order_count = 0; // counts number of offers in data set, ensures we only process the order once
    vector<order_info> order_info_vec, my_order_info; // my_order_info -> used to store information pertaining to user's order

    hours hrs(9);
    minutes mns(45), mns2(40);
    milliseconds time(1), nine_forty_five_AM = duration_cast<milliseconds>(hrs) + duration_cast<milliseconds>(mns), nine_forty_AM = duration_cast<milliseconds>(hrs) + duration_cast<milliseconds>(mns2);

    Order sample_buy3(100010,4, 300, Stock, "IBM", milliseconds(300), Limit, Day, Buy, 153.83);

    cout << "Reached while loop " << endl;

        my_broker.Update_OrderTable(sample_buy3,con,long(5000));

    while(getline(read_data, line) && time.count() < nine_forty_five_AM.count()){
        Offer temp = create_offer_with_dest(line);
        order_info_vec = route_offer(temp, All_ECNs);
                time = temp.timestamp;
                       if(time.count() >= nine_forty_AM.count() && order_count < 1){
                            my_order_info = my_broker.ProcessOrder(sample_buy3, con, long(5000));
//                            my_broker.update_orders(order_info_vec, con,)
                            order_count++;
                        }
        if(order_info_vec.size() > 0){
            cout << "error "<< endl;
   ...
   cout << "total number of offers: " <<  num_of_offers << endl;
...
//    StockMarketSimulation(sample_ask2);
}
...
//Order sample_buy3(100010,4, 300, Stock, "IBM", milliseconds(300), Limit, Day, Buy, 153.83);
//
//int order_count = 0; //ensures we only process the order once
//vector<order_info> order_info_vec, my_order_info;
//while(getline(read_data, line) && time.count() < nine_forty_five_AM.count()){
//    Offer temp = create_offer_with_dest(line);
//    order_info_vec = route_offer(temp, All_ECNs);
//    time = temp.timestamp;
//    if(time.count() >= nine_forty_AM.count() && order_count < 1){
//        my_order_info = my_broker.ProcessOrder(sample_buy3);
//        order_count++;
//    }
//    if(order_info_vec.size() > 0)
//        count++;
...
// Why you shouldn't use system("CLS") or system("PAUSE") http://www.cplusplus.com/forum/articles/11153/
// Methods for clearing console screen: http://www.cplusplus.com/forum/articles/10515/
