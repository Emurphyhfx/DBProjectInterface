...
public:
    ...
    class Market market; // we will have to create a data structure to hold many market objects if we are to create an ECN which trades many stocks.
    ...
    vector<order_info> ParseOffer_TEST(Offer &offer); // this function will parse offers, basically decides if the offer to be added can be used to form a transaction given the current bids and asks or if the offer should be inserted.
