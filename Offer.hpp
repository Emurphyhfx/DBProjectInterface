...
public:
    ...
    Term OfferTerm; // AON, FOK, GTC, etc ??
    ...
    // Can delete this one after we get everything working
    Offer(float P, int Q, Type t, milliseconds ms){ // for testing out bids and asks in the ECN
        Price = P;
        Qty = Q;
        OfferType = t;
        timestamp = ms;
...
//    const Offer operator = (const Offer &rhs);
