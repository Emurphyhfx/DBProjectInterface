...
//  DBMS_Playground
//
//  Created by Thomas Ciha on 2/15/18.
...
enum OrdType{
    Limit, Market, MarketonClose, StoponQuote, StopLimitonQuote, HiddenStop, TrailingStop_Dollar, TrailingStop_Percent
};


/* BELOW is a breakdown of how each order type will be dealt with in our application:
...
enum Status{
    Filled, Pending, Canceled, Aborted
};
...
enum Method{
    NA, Internalized, ECN //NA - Not Applicable
};


// Need to add an attribute, "Order Fulfillment Method", which is initially NULL, but specifies how the order was filled once the order has been completed (i.e. what exchange filled the order, was the order internalized by the broker, was it sent to an ECN)

class Order{
    public:
    long OrderNumber, AccountNumber;
    float Price, fill_price; // price = price entered, fill_price = price received
    int Size;
    ...
    //constructor
    Order(long ON, long AN, int S, Class AC, string T, milliseconds Time, OrdType Ty, Term Trm, Action Act, float price){
        OrderNumber = ON;
        AccountNumber = AN;
        Size = S;
        AssetClass = AC;
        ...
        OrderTerm = Trm;
        OrderAction = Act;
        OrderFulfillmentMethod = NA; //default value of NA, this will be changed once the order has been filled
        Price = price;
        fill_price = 0;
        OrderStatus = Pending;
    }
    ...
    void Print(){
        ...
        cout << "Order Action: ";
        switch(OrderAction){                              // NOTE: there are SEVERAL more order types, rn only dealing with limit and market orders
