...
pair<bool, string> OMS::eligible_order(Order &o, SAConnection &con, long AccNum){
   ...
   //  input: account number, ticker
    ...
    get_eligibility_info_stock << o.Ticker.c_str() << AccNum;
    ...
    if(get_eligibility_info_stock.FetchNext()){
     ...
     if(o.OrderAction == Cover){
        ...
        if(get_short_info.FetchNext()){ // if the user has a short position
            ...
            if(llabs(short_qty) < o.Size) // if attempting to cover more shares in the short position, set order size to num of shares shorted
                o.Size = llabs(short_qty);
        }
        else
     ...
     Type offer_type;
     (o.OrderAction == Buy || o.OrderAction == Cover) ? offer_type = Bid : offer_type = Ask;

        if(offer_type == Bid){      // if it's a buy or cover order (which results in an offer of type Bid
            if(o.OrderType == Limit){
                if(cash_balance >= commission_rate + (o.Size * o.Price)){
                ...
                if(cash_balance >= commission_rate + (o.Size * get_best_mkt_price(offer_type).first))
                    return make_pair(true, "");
                else{
            ...
            if(o.OrderAction == Short)
                return make_pair(true, "");
            // Parsing Sell Order
            if(o.OrderAction == Sell && num_shares_owned == 0)
                return make_pair(false, "we cannot sell any shares that we do not own, we can only short sell if we own no shares");

            if(num_shares_owned < o.Size) // default condition as described above ^
                o.Size = num_shares_owned;
            }
    }
    else
...
void update_order_table_query(SAConnection &con, Order &o, long AccNum){
        ...
        update_order_table.Param(13).setAsDouble() = o.Price;
...
//     SQL QUERY HERE
    // input: order, AccountNo     WE NEED TO INTEGRATE THE USER OBJECT INTO APPLICATION
    ...
    if(o.OrderAction == Buy || o.OrderAction == Cover){
    ...
    //===== UPDATE ORDER TABLE ======
    //cout << "Before updating, fill price = " << o.fill_price << " entered price = " << o.Price << endl;
    if(o.OrderType == Market && (o.OrderAction == Buy || o.OrderAction == Cover)) // if we changed o.Price to MAX FLOAT for processing purposes,
        o.Price = NULL; // entered price cannot be stored as MAX FLOAT in DBMS and for market, change back to NULL
  ...
  if(o.OrderAction == Buy || o.OrderAction == Sell) {
    cout << "Checking contains stock " << endl; // we are updating ContainsStock
    check_contains_stock << AccNum << o.Ticker.c_str();
    check_contains_stock.Execute();
     if(check_contains_stock.FetchNext()){ // if the user has a position in ContainsStock, adjust it accordingly
        ...
        if(o.OrderAction == Buy){
              new_qty = o.Size + num_shares; // we initialized num_shares = 0, so if no position this still works
              new_avg_price = ((avg_price * num_shares) + (o.fill_price * o.Size)) / new_qty;
          }
            ...
            if(new_qty != 0){
               if(already_own_some){
                update_contains_stock << new_qty << new_avg_price << o.Ticker.c_str() << AccNum;
                update_contains_stock.Execute();
               }
                else {
                    string asset_class = "Stock";
                    insert_stock_position << AccNum << o.Ticker.c_str() << (double) o.fill_price << (long) o.Size << asset_class.c_str();
                    insert_stock_position.Execute();
                }
            }
            else { //remove position from contains stock
                remove_stock_position << AccNum << o.Ticker.c_str();
        ...
        check_contains_short << AccNum << o.Ticker.c_str();
    ...
    if(new_qty != 0){
        if(already_own_some){
            ...
            insert_short_position << AccNum << o.Ticker.c_str() << (double) o.fill_price << insertion_qty << asset_class.c_str();
            insert_short_position.Execute();
        }
    }
    else { //remove the position from contains short
        remove_short_position << AccNum << o.Ticker.c_str();
    ...
    pair<bool, string> eligibility_info = eligible_order(o,con, AccNum);
     ...
     if(eligibility_info.first){
        ...
        if(o.OrderType == Market &&(o.OrderAction == Sell || o.OrderAction == Short))
            o.Price = 0;                    // no entered price with market orders
        else if(o.OrderType == Market && (o.OrderAction == Buy || o.OrderAction == Cover))
            o.Price = std::numeric_limits<float>::max();

        pending_orders.push_back(o);        // add to pending orders
        Offer new_offer(o.Price, o.Size, o.timestamp, o.OrderTerm, o.OrderType, true, o.OrderNumber); // true means the offer was placed from the OMS
        (o.OrderAction == Buy || o.OrderAction == Cover) ? new_offer.OfferType = Bid : new_offer.OfferType = Ask; // setting appropriate offer type


        // (o.OrderType == MarketonClose) ? end_of_day.push(o); // handles market on close orders
        // return order_information;

        // ===== DECIDING IF INTERNALIZING IS PROFITABLE && POSSIBLE ======== (see #1)


/*

        // get internalization price, (it will be MAXFLOAT if we cannot internalize it)
        pair<float, int> internal_inventory = get_internalization_price_and_qty(o.Ticker, con, AccNum); // see #2
        ...
        bool internalization_is_profitable = (new_offer.OfferType == Bid) ? (internal_inventory.first < o.Price) : false; // Note: see #3
        ...
        if(internalization_is_viable && internalization_is_profitable){ //we internalize the order
            ...
            (o.OrderType == Market) ? internalization_price = best_mkt_price_and_location.first - .01 : internalization_price = o.Price;

            order_info internalization_info = internalize_order(new_offer, internal_inventory, internalization_price, o.Ticker, con, AccNum);
            ...
            */
            ...
            pair<float, int> best_mkt_price_and_location = get_best_mkt_price(new_offer.OfferType); // for test purposes (location is here)
            ...
            while(o.OrderType == Market && order_information.end()->incomplete_market_order == true) {
        ...
        if(order_information.size() > 0)
            ...
            //Limit, Market, MarketonClose, StoponQuote, StopLimitonQuote, HiddenStop, TrailingStop_Dollar, TrailingStop_Percent -- OrderType
            //    AON, GTC, IOC, FOK, Day - Term

        //some orders will be completely processed right away, others with type == MarketonClose will be sent at the end of the day. doing this will be easy, we just empty a queue once the timestamp of the offers reaches 4:30pm.

     }

    else {
        cout << "Order " << o.OrderNumber << " Ineligible: " << eligibility_info.second << endl;
        o.OrderStatus = Aborted; // probably need to add this to our order status domain in DB to ensure we don't violate referential integrity constraints
    }
    return order_information;
}
